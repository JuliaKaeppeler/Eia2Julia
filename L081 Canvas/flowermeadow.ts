namespace flowermeadow {
    ...
    function drawFirTree(_position: Vector): void {
    ...
    function drawRose(_position: Vector): void {
    ...
    function drawStarflower(_position: Vector): void {
    ...
    function drawSun(_position: Vector): void { // Sonne erwartet Position, die vom Typ Vector ist
    ...
    function drawCloud(_position: Vector, _size: Vector): void { // Erwartet ebenfalls die Position, aber auch die Größe. Jeweils vom Typ Vector
        ...
        for (let drawn: number = 0; drawn < nParticles; drawn++) { 
            ...
            let x: number = (Math.random() - 0.5) * _size.x; // x soll eine number sein und ist gleich ein Zufallswert zwischen 0 und 1. Da ziehe ich 0.5 ab, dann habe ich einen Wert zwischen -0.5 und +0.5. Wichtig ist Klammern wegen Punkt vor Strich. Undd dann multipliziere ich das mit der Größe.
            let y: number = - (Math.random() * _size.y); //Zufallszahl zwischen 0 und 1 und das kann ich mit der Ausdehnung der Vertikalen multipliziern. Aber ich brauche den negativen Wert deswegen - (Math.Random()..)
            crc2.translate(x, y); 
